...
class score:
	PP_CALCULATORS = {
		gameModes.STD: autooppai.oppai,
		gameModes.TAIKO: autooppai.oppai,
	...
	def setDataFromDB(self, scoreID, rank = None):
		...
		"""
		data = glob.db.fetch("SELECT scores_auto.*, users.username FROM scores_auto LEFT JOIN users ON users.id = scores_auto.userid WHERE scores_auto.id = %s LIMIT 1", [scoreID])
		if data is not None:
	...
	def saveScoreInDB(self):
		...
		if self.completed >= 2:
			...
			if self.oldPersonalBest != 0:
				glob.db.execute("UPDATE scores_auto SET completed = 2 WHERE id = %s", [self.oldPersonalBest])
	...
	def calculatePP(self, b = None):
		...
		# Calculate pp
		if b.rankedStatus >= rankedStatuses.RANKED and b.rankedStatus != rankedStatuses.PENDING and b.rankedStatus != rankedStatuses.UNKNOWN \
