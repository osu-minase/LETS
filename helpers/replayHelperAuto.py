...
def buildFullReplay(scoreID=None, scoreData=None, rawReplay=None):
    ...
    if scoreData is None:
        scoreData = glob.db.fetch(
            "SELECT scores_auto.*, users.username FROM scores_auto LEFT JOIN users ON scores_auto.userid = users.id "
            "WHERE scores_auto.id = %s",
            [scoreID]
        )
    else:
    ...
    if rawReplay is None:
        ...
        fileName = ".data/replays_auto/replay_{}.osr".format(scoreID)
